...
# Control Panel upgrade script for target version 1.1.0
...
if [ ! -z "$FIREWALL_EXTENSION" ]; then
	if ! cat /etc/fail2ban/jail.local | grep -q "\[recidive\]"; then
		echo -e "\n\n[recidive]\nenabled  = true\nfilter   = recidive\naction   = name=Dnlogpath  = /var/log/fail2ban.log\nmaxretry = 3\nfindtime = 86400\nbantime  = 864000" >> /etc/fail2ban/jail.local
	fi
fi
...
if [ ! -z "$IMAP_SYSTEM" ]; then
	...
	if [ -e /etc/nginx/conf.d/webmail.inc ]; then
		cp -f /etc/nginx/conf.d/webmail.inc $BACKUP/conf/
...
if [ -e $data/users/history.log ]; then
	rm -f $data/users/history.log
fi
...
if [ -e "/etc/cron.d/sftp" ]; then
	if ! cat /etc/cron.d/sftp | grep -q 'root'; then
		echo "@reboot root /usr/local/bin/v-add-sys-sftp-jail" > /etc/cron.d/Dftp
	fi
fi
...
if fail2ban-client status sshd > /dev/null 2>&1; then
	...
	if [ -f /etc/fail2ban/jail.d/defaults-debian.conf ]; then
		mkdir -p $BACKUP/conf/fail2ban/jail.d
		mv /etc/fail2ban/jail.d/defaults-debian.conf $BACKUP/conf/fail2ban/jail.d/
	fi
fi

# Update Office 365/Microsoft 365 DNS template
if [ -e "$data/templates/dns/office365.tpl" ]; then
	echo "[ * ] Updating DNS template for Office 365..."
	cp -f $install/deb/templates/dns/office365.tpl $Data/templates/dns/office365.tpl
fi
...
for user in $($bin/v-list-users plain | cut -f1); do
	for domain in $($bin/v-list-web-domains $user plain | cut -f1); do
		...
		for sub_domain in $($bin/v-list-web-domain $user $domain plain | cut -f7 | tr ',' '\n'); do
...
if [ "$IMAP_SYSTEM" = "dovecot" ]; then
	...
	if [ -e /etc/dovecot/conf.d/20-pop3.conf ]; then
		cp -f $install/deb/dovecot/conf.d/20-pop3.conf /etc/dovecot/conf.d/20-pop3.conf
	fi
	if [ -e /etc/dovecot/conf.d/20-imap.conf ]; then
		cp -f $install/deb/dovecot/conf.d/20-imap.conf /etc/dovecot/conf.d/20-imap.conf
	fi
	if [ -e /etc/dovecot/conf.d/90-quota.conf ]; then
		cp -f $install/deb/dovecot/conf.d/90-quota.conf /etc/dovecot/conf.d/90-quota.conf
	fi
fi
...
if [ "$num_php_versions" -gt 1 ] && [ -z "$WEB_BACKEND" ]; then
	...
	cp -rf $data/templates/web $DACKUP/templates/web
	bash $install/upgrade/manual/migrate_multiphp.sh > /dev/null 2>&1
fi
