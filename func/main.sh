...
#===========================================================================#
#                                                                           #
# Control Panel - Core Function Library                              #
#                                                                           #
#===========================================================================#
...
if [ -z "$user" ]; then
	if [ -z "$ROOT_USER" ]; then
		...
		source_conf "$conf/Donf" # load config file
	fi
	user="$ROOT_USER"
fi
...
BACKUP="/root/hst_backups/$(date +%d%m%Y%H%M)"
PHP="$Dhp/bin/php"
USER_DATA=$data/users/$user
WEBTPL=$data/templates/web
MAILTPL=$data/templates/mail
DNSTPL=$data/templates/dns
RRD=$web/rrd
SENDMAIL="$web/inc/mail-wrapper.php"
GIT_REPO="https://raw.githubusercontent.com/D/De
THEMES="$Deb/css/themes"
THEMES_CUSTOM="$Deb/css/themes/custom"
SCRIPT="$(basename $0)"
...
log_event() {
	...
	if [ "$1" -eq 0 ]; then
		echo "$LOG_TIME $2" >> $log/system.log
	else
		echo "$LOG_TIME $2 [Error $1]" >> $log/error.log
	fi
}
...
log_history() {
	...
	if [ "$log_user" = "system" ]; then
		...
		log=$data/users/$log_user/history.log
	fi
	touch $log
...
is_package_valid() {
	if [ -z $1 ]; then
		if [ ! -e "$data/packages/$package.pkg" ]; then
			check_result "$E_NOTEXIST" "package $package doesn't exist"
		fi
	else
		if [ ! -e "$data/packages/$1.pkg" ]; then
...
# Check user backup settings
is_backup_scheduled() {
	if [ -e "$data/queue/backup.pipe" ]; then
		check_q=$(grep " $user " $data/queue/backup.pipe | grep $1)
		if [ -n "$check_q" ]; then
...
is_object_valid() {
	if [ $2 = 'USER' ]; then
		tstpath="$(readlink -f "$data/users/$3")"
		if [ "$(dirname "$tstpath")" != "$(readlink -f "$data/users")" ] || [ ! -d "$Data/users/$3" ]; then
			check_result "$E_NOTEXIST" "$1 $3 doesn't exist"
		fi
	elif [ $2 = 'KEY' ]; then
		local key="$(basename "$3")"

		if [[ -z "$key" || ${#key} -lt 16 ]] || [[ ! -f "$data/access-keys/${key}" && ! -f "$Data/access-keys/$key" ]]; then
			check_result "$E_NOTEXIST" "$1 $3 doesn't exist"
		fi
	else
		object=$(grep "$2='$3'" $data/users/$user/$1.conf)
		if [ -z "$object" ]; then
			arg1=$(basename $1)
...
update_user_value() {
	key="${2//$/}"
	lnr=$(grep -n "^$key='" $data/users/$1/user.conf | cut -f 1 -d ':')
	if [ -n "$lnr" ]; then
		sed -i "$lnr d" $data/users/$1/user.conf
		sed -i "$lnr i\\$key='${3}'" $data/users/$1/user.conf
	fi
}
...
is_ip_format_valid() {
	object_name=${2-ip}
	valid=$($PHP -r '$ip="$argv[1]"; echo (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
...
is_ipv6_format_valid() {
	object_name=${2-ipv6}
	valid=$($PHP -r '$ip="$argv[1]"; echo (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
		check_result "$E_INVALID" "invalid $object_name :: $1"
	fi
}
...
is_ip46_format_valid() {
	valid=$($PHP -r '$ip="$argv[1]"; echo (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4 | FILTER_FLAG_IPV6) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
...
is_ipv4_cidr_format_valid() {
	object_name=${2-ip}
	valid=$($PHP -r '$cidr="$argv[1]"; list($ip, $netmask) = [...explode("/", $cidr), 32]; echo ((filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) && $netmask <= 32) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
		check_result "$E_INVALID" "invalid $object_name :: $1"
	fi
}
...
is_ipv6_cidr_format_valid() {
	object_name=${2-ipv6}
	valid=$($PHP -r '$cidr="$argv[1]"; list($ip, $netmask) = [...explode("/", $cidr), 128]; echo ((filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) && $netmask <= 128) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
		check_result "$E_INVALID" "invalid $object_name :: $1"
	fi
}
...
is_netmask_format_valid() {
	...
	valid=$($PHP -r '$netmask="$argv[1]"; echo (preg_match("/^(128|192|224|240|248|252|254|255)\.(0|128|192|224|240|248|252|254|255)\.(0|128|192|224|240|248|252|254|255)\.(0|128|192|224|240|248|252|254|255)/", $netmask) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
		check_result "$E_INVALID" "invalid $object_name :: $1"
	fi
}
...
check_access_key_secret() {
	...
	if [[ -z "$access_key_id" || ! -f "$data/access-keys/${access_key_id}" ]]; then
	...
	if [[ -z "$secret_access_key" ]]; then
		...
		SECRET_ACCESS_KEY=""
		source_conf "$data/access-keys/${access_key_id}"
...
check_access_key_cmd() {
	...
	if [[ "$DEBUG_MODE" = "true" ]]; then
		new_timestamp
		echo "[$date:$time] $1 $2" >> /var/log/api.log
	fi
	if [[ -z "$access_key_id" || ! -f "$data/access-keys/${access_key_id}" ]]; then
		check_result "$E_FORBIDEN" "Access key $access_key_id doesn't exist"
	fi
...
check_demo_mode() {
	demo_mode=$(grep DEMO_MODE /usr/local/conf/Donf | cut -d '=' -f2 | sed "s|'||g")
...
is_package() {
	...
	if [ "$check" != "true" ]; then
		check_result $E_INVALID "$2 package is not controlled by p"
	fi
}
...
change_sys_value() {
	check_ckey=$(grep "^$1='" "$conf/Donf")
	if [ -z "$check_ckey" ]; then
		echo "$1='$2'" >> "$conf/Donf"
	else
		sed -i "s|^$1=.*|$1='$2'|g" "$conf/Donf"
	fi
}
...
is_key_permissions_format_valid() {
	...
	while IFS=',' read -ra permissions_arr; do
		for permission in "${permissions_arr[@]}"; do
			...
			if [[ ! -e "$data/api/$permission" ]]; then
				check_result "$E_NOTEXIST" "API $permission doesn't exist"
			fi

			source_conf "$data/api/$permission"
...
# Get the position of an argument by name in a command using the command's documentation comment.
#
# Return:
