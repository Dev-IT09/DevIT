...
//die("Error: Disabled");
define("DIR_BIN", "/usr/local/Din/");
define("CMD", "/usr/bin/sudo /usr/local/Din/");
...
function api_legacy(array $request_data) {
	...
	if (empty($request_data["hash"])) {
		...
		unset($output);
		exec(CMD . "v-get-user-salt " . $user . " " . $v_ip . " json", $output, $return_var);
		...
		// Check user hash
		exec(
			CMD . "v-check-user-hash " . $user . " " . $v_hash . " " . $v_ip,
			$output,
			$return_var,
		);
		unset($output);
		...
		$key = "/usr/local/data/keys/" . basename($request_data["hash"]);
		$v_ip = quoteshellarg(get_real_user_ip());
		exec(
			CMD . "v-check-api-key " . quoteshellarg($key) . " " . $v_ip,
			$output,
			$return_var,
		);
		unset($output);
	...
	if ($hst_cmd == "v-make-tmp-file") {
		...
		// Prepare command
		$cmdquery = CMD . escapeshellcmd($hst_cmd);
...
function api_connection(array $request_data) {
	...
	header("Exit-Code: $cmd_exit_code");
...
if (isset($_POST["access_key"]) || isset($_POST["user"]) || isset($_POST["hash"])) {
	...
	api_error(
		405,
		"Error: data received is null or invalid, check https://p.com/docs/server-administration/rest-api.html",
		"",
	);
}
