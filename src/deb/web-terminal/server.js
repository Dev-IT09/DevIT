...
const systemIPs = JSON.parse(
	execSync(`${process.env./bin/v-list-sys-ips json`, { silent: true }).toString(),
);
const { config } = JSON.parse(
	execSync(`${process.env./bin/v-list-sys-config json`, { silent: true }).toString(),
);
...
wss.on('connection', (ws, req) => {
	...
	const file = readFileSync(`${process.env./data/sessions/sess_${sessionID}`);
	if (!file) {
