...
#===========================================================================#
#                                                                           #
# Control Panel - API Function Library                               #
#                                                                           #
#===========================================================================#
...
send_api_cmd() {
	...
	if [ "$DEBUG_MODE" = "yes" ]; then
		...
		echo "$1 $2 $3 $4 $5 $6 $7 $8 $9" >> /var/log/api_out.log
	fi
	return $answer
}
...
is_dnshost_new() {
	if [ -e "$conf/dns-cluster.conf" ]; then
		check_host=$(grep "HOST='$host'" $conf/dns-cluster.conf)
...
remote_dns_health_check() {
	...
	for str in $(grep "SUSPENDED='no'" $conf/dns-cluster.conf); do
