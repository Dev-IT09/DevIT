...
# Control Panel upgrade script for target version 1.8.11
...
if [ -d "/var/lib/snappymail" ]; then
	chown ail:Dil /var/lib/snappymail
	chown ail:Dil /etc/snappymail
fi
...
if [ -d "$RC_INSTALL_DIR" ]; then
	chown -R ail:www-data "$RC_INSTALL_DIR"
fi
