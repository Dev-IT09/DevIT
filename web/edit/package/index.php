...
exec(CMD . "v-list-user-package " . $v_package . " 'json'", $output, $return_var);
...
// List dns templates
exec(CMD . "v-list-dns-templates json", $output, $return_var);
...
if (!empty($_POST["save"])) {
	...
	fwrite($fp, $pkg);
	exec(
		CMD . "v-add-user-package " . $tmpfile . " " . $v_package . " yes",
		$output,
		$return_var,
	);
	...
	exec(CMD . "v-update-user-package " . $v_package . " 'json'", $output, $return_var);
	check_return_code($return_var, $output);
	unset($output);

	if ($v_package_new != $v_package) {
		exec(
			CMD . "v-rename-user-package " . $v_package . " " . $v_package_new,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
