...
#===========================================================================#
#                                                                           #
# Control Panel - Backup Function Library                            #
#                                                                           #
#===========================================================================#
...
local_backup() {
	...
	if [ "$disk_usage" -ge "$BACKUP_DISK_LIMIT" ]; then
		...
		sed -i "/ $user /d" $data/queue/backup.pipe
	...
	chown "eb":"$user" $BACKUP/$user.$backup_new_date.tar
	localbackup='yes'
...
ftp_backup() {
	# Checking config
	if [ ! -e "$conf/ftp.backup.conf" ]; then
	...
	# Parse config
	source_conf "$conf/ftp.backup.conf"

	# Set default port
	if [ -z "$(grep 'PORT=' $conf/ftp.backup.conf)" ]; then
		PORT='21'
	fi
...
sftp_download() {
	source_conf "$conf/sftp.backup.conf"
	if [ -z "$PORT" ]; then
		PORT='22'
	fi
	cd $BACKUP
...
sftp_backup() {
	# Checking config
	if [ ! -e "$conf/sftp.backup.conf" ]; then
	...
	# Set default port
	if [ -z "$(grep 'PORT=' $conf/sftp.backup.conf)" ]; then
		PORT='22'
	fi
...
b2_backup() {
	...
	source_conf "$conf/b2.backup.conf"
...
rclone_backup() {
	...
	source_conf "$conf/rclone.backup.conf"
