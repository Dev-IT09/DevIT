...
mysql_connect() {
	unset PORT
	host_str=$(grep "HOST='$1'" $conf/mysql.conf)
	...
	if [ '0' -ne "$?" ]; then
		if [ "$notify" != 'no' ]; then
			email=$(grep CONTACT "$data/users/$ROOT_USER/user.conf" | cut -f 2 -d \')
...
psql_connect() {
	unset PORT
	host_str=$(grep "HOST='$1'" $conf/pgsql.conf)
	parse_object_kv_list "$host_str"
...
is_charset_valid() {
	host_str=$(grep "HOST='$host'" $conf/$type.conf)
	parse_object_kv_list "$host_str"
...
increase_dbhost_values() {
	...
	sed -i "s/$old_dbbases/$new_dbbases/g" $conf/$type.conf
	sed -i "s/$old_users/$new_users/g" $conf/$type.conf
}
...
is_dbhost_new() {
	if [ -e "$conf/$type.conf" ]; then
		check_host=$(grep "HOST='$host'" $conf/$type.conf)
		if [ "$check_host" ]; then
