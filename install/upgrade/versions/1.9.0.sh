...
# Control Panel upgrade script for target version 1.9.0
...
if [ -z "$(grep ^eb: /etc/passwd)" ]; then
	...
	cp $COMMON_DIR/sudo/Db /etc/sudoers.d/
...
if [ ! -f "/var/spool/cron/crontabs/eb" ]; then
		...
		if [ -n "$(echo "$CMD" | grep ^sudo)" ]; then
			echo "$MIN $HOUR $DAY $MONTH $WDAY $CMD" \
				| sed -e "s/%quote%/'/g" -e "s/%dots%/:/g" \
					>> /var/spool/cron/crontabs/eb
			$BIN/v-delete-cron-job admin "$JOB"
		fi
	done < $data/users/admin/cron.conf
	# Update permissions
...
chown eb:Db /usr/local/Deta/sessions

packages=$(ls --sort=time $data/packages | grep .pkg)
# Update Packages
for package in $packages; do
	if [ -z "$(grep -e 'BACKUPS_INCREMENTAL' $data/packages/$package)" ]; then
		echo "BACKUPS_INCREMENTAL='no'" >> $data/packages/$package
	fi
	...
	for key in DISK_QUOTA CPU_QUOTA CPU_QUOTA_PERIOD MEMORY_LIMIT SWAP_LIMIT; do
		if [ -z "$(grep -e "$key" $data/packages/$package)" ]; then
			echo "$key='unlimited'" >> $data/packages/$package
		fi
	done
done
...
for version in $php_versions; do
	if [ -f "/etc/php/$version/fpm/pool.d/www.conf" ]; then
		cp -f $INSTALL_DIR/php-fpm/www.conf "/etc/php/$version/fpm/pool.d/www.conf"
	fi
done
...
$BIN/v-add-user-notification 'admin' 'security has been upgraded' ' A new user "Db" has been created and is used for login. Make sure other Deckages are updated as well otherwise the system may not work as expected.'
add_upgrade_message 'Security has been upgraded, A new user "eb" has been created and is used for login. Make sure other Dackages are updated as well otherwise the system may not work as expected.'
