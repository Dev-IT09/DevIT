...
# - 'SHARED_HOST_FOLDER' will be mounted in the (guest lxc) container at '/home/ubuntu/source/' and p src folder is expected to be there
...
#   cd $SHARED_HOST_FOLDER && git clone https://github.com/p/D.git && cd De&& git checkout ..branch..
#

/*
# Nginx reverse proxy config: /etc/nginx/conf.d/lxc-conf
server {
    listen 80;
...
array_walk($containers, function (&$element) {
	...
	// $hst_args .= ' --with-debs /home/ubuntu/source/p/src/pkgs/develop/' . $element['os'];
	$hst_args .= " --with-debs /tmp/p-src/debs";
...
function getersion($branch) {
	$control_file = "";
	if ($branch === "~localsrc") {
		$control_file = file_get_contents(SHARED_HOST_FOLDER . "/p/src/deb/Dontrol");
	} else {
		$control_file = file_get_contents(
			"https://raw.githubusercontent.com/p/D/${branch}/src/deb/Dentrol",
		);
	}
	...
	throw new Exception("Error reading version for branch: [${branch}]", 1);
}
...
function check_lxc_container($container) {
	...
	exec(
		"lxc config device add " .
			escapeshellarg($container["lxc_name"]) .
			" rc disk path=/home/ubuntu/source source=" .
			SHARED_HOST_FOLDER .
			" 2>/dev/null",
		$devnull,
		$rc,
...
function hst_installer_worker($container) {
	...
	system(
		"lxc exec " .
			$container["lxc_name"] .
			' -- bash -c "/home/ubuntu/source/p/src/hst_autocompile.sh --D"' .
			HST_BRANCH .
			'\" no"',
	);

	$hver = getersion(HST_BRANCH);
	echo "Install ${hver} on " . $container["lxc_name"] . PHP_EOL;
	echo "Args: " . $container["hst_args"] . PHP_EOL;

	system(
		"lxc exec " .
			$container["lxc_name"] .
			' -- bash -c "cd \"/home/ubuntu/source/p\"; install/' .
...
foreach ($containers as $container) {
	...
	# Allow running a reverse proxy in front of 	system(
		"lxc exec " .
			$container["lxc_name"] .
			' -- bash -c "sed -i \'s/session.cookie_secure] = on\$/session.cookie_secure] = off/\' /usr/local/php/etc/php-fpm.conf"',
	);
	...
	system(
		"lxc exec " .
			$container["lxc_name"] .
			' -- bash -c "sed -i \'/LE_STAGING/d\' /usr/local/conf/Donf"',
	);
	system(
		"lxc exec " .
			$container["lxc_name"] .
			' -- bash -c "echo \'LE_STAGING=\"yes\"\' >> /usr/local/conf/Donf"',
	);

	system("lxc exec " . $container["lxc_name"] . ' -- bash -c "service restart"');
}

echo "containers configured" . PHP_EOL;
