...
if ($selected_theme !== "default") {
	// Load P-shipped themes (minified, updated/overwritten with updates) - ($Deb/css/themes/*.min.css)
	$non_default_theme_path = $_SERVER["] . "/web/css/themes/" . $selected_theme . ".min.css";
	if (file_exists($non_default_theme_path)) {
		echo '<link rel="stylesheet" href="/css/themes/' . $selected_theme . ".min.css?" . JS_LATEST_UPDATE . '">';
	}
	// Load custom theme files ($web/css/themes/custom/*.css)
	else {
		$custom_theme_path = $_SERVER["] . "/web/css/themes/custom/" . $selected_theme . ".min.css";
