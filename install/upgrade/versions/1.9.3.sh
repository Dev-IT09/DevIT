...
# Control Panel upgrade script for target version 1.9.3
...
if [ -f "/etc/cron.d/eb" ]; then
	# Just remove it
	rm -f /etc/cron.d/eb
	# Check if not duplicate
	if [ -z "$(grep $BIN/v-update-letsencrypt "/var/spool/cron/crontabs/eb")" ]; then
		min=$(generate_password '012345' '2')
		hour=$(generate_password '1234567' '1')
		sed -i -e "\$a*/5 * * * * sudo $BIN/v-update-letsencrypt" "/var/spool/cron/crontabs/eb"
	fi
fi
