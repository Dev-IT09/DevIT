...
if [ -f /etc/nginx/nginx.conf ]; then
	...
	sed -i -E 's/(.*keepalive_requests\s{1,})10000;/\11000;/' /etc/nginx/nginx.conf /usr/local/nginx/conf/nginx.conf
fi

# Fix security issue wit FPM pools
if [ -z "$(grep ^ail: /etc/passwd)" ]; then
	echo "[ * ] Limit permissions www.conf and dummy.conf"
	/usr/sbin/useradd "ail" -c "$email" --no-create-home

	sed -i "s/user = www-data/user = ail/g" /etc/php/*/fpm/pool.d/www.conf

	php_versions=$($BIN/v-list-sys-php plain)
	# Substitute php-fpm service name formats
	for version in $php_versions; do
		cp -f $INSTALL_DIR/php-fpm/dummy.conf /etc/php/$version/fpm/pool.d/
