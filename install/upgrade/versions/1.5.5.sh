...
# Control Panel upgrade script for target version 1.5.5
...
for version in $($BIN/v-list-sys-php plain); do
	cp -f $INSTALL_DIR/php-fpm/php-fpm.conf /etc/php/$version/fpm/
...
if [ ! -f "$data/packages/system.pkg" ]; then
	echo "[ * ] Install default system package."
	cp -f $install/deb/packages/system.pkg $Data/packages/system.pkg
fi
