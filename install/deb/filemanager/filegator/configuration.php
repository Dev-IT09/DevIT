...
$dist_config["frontend_config"]["app_name"] = "File Manager - Control Panel";
...
// Update list of languages when new language is added on or Filegator side
switch ($lang) {
	case "es":
...
$dist_config["services"]["Filegator\Services\Storage\Filesystem"]["config"][
	...
	if (!empty($_SESSION["INACTIVE_SESSION_TIMEOUT"])) {
		if ($_SESSION["INACTIVE_SESSION_TIMEOUT"] * 60 + $_SESSION["LAST_ACTIVITY"] < time()) {
			...
			exec(
				"/usr/local/bin/v-log-user-logout " . $v_user . " " . $v_session_id,
				$output,
				$return_var,
			);
			unset($_SESSION);
	...
	if (!file_exists("/home/" . basename($v_user) . "/.ssh/hst-filemanager-key")) {
		exec(
			"sudo /usr/local/bin/v-add-user-sftp-key " .
	...
	if (!isset($_SESSION["SFTP_PORT"])) {
		exec("sudo /usr/local/bin/v-list-sys-sshd-port json", $output, $result);
...
$dist_config["services"]["Filegator\Services\Archiver\ArchiverInterface"] = [
	"handler" => "\Filegator\Services\Archiver\Adapters\ipArchiver",
	"config" => [],
];

$dist_config["services"]["Filegator\Services\Auth\AuthInterface"] = [
	"handler" => "\Filegator\Services\Auth\Adapters\uth",
	"config" => [
