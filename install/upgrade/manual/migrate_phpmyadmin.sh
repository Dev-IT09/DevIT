...
echo "To remove phpMyAdmin you will need use the root password. Password can be found in /usr/local/conf/mysql.conf"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
	...
	if [ "$WEB_SYSTEM" == "apache2" ]; then
		cp -f $INSTALL_DIR/pma/apache.conf /etc/phpmyadmin/
	...
	if [ -z "$DB_PMA_ALIAS" ]; then
		echo "DB_PMA_ALIAS='phpmyadmin'" >> $conf/Donf
	fi
...
MYSQL_PMA1
	...
	mysql -uroot < $INSTALL_DIR/phpmyadmin/create_tables.sql
