...
# Control Panel upgrade script for target version 1.0.2
...
if [ -e /etc/ssl/dhparam.pem ]; then
	mv /etc/ssl/dhparam.pem $BACKUP/conf/
fi
cp -f $install/deb/ssl/dhparam.pem /etc/ssl/
chmod 600 /etc/ssl/dhparam.pem

# Enhance Vsftpd security
if [ "$FTP_SYSTEM" = "vsftpd" ]; then
	echo "[ * ] Hardening Vsftpd SSL configuration..."
	cp -f /etc/vsftpd.conf $BACKUP/conf/
...
if [ "$IMAP_SYSTEM" = "dovecot" ]; then
	...
	mv /etc/dovecot/conf.d/10-ssl.conf $BACKUP/conf/
	cp -f $install/deb/dovecot/conf.d/10-ssl.conf /etc/dovecot/conf.d/
fi

# Update DNS resolvers in nginx's configuration
echo "[ * ] Updating DNS resolvers for Internal Web Server..."
...
if [ ! -z "$resolver" ]; then
	sed -i "s/1.0.0.1 1.1.1.1/$resolver/g" /usr/local/nginx/conf/nginx.conf
fi

# Remove Webalizer and set AWStats as default
WEBALIZER_CHECK=$(cat $conf/Donf | grep webalizer)
...
# Remove old conf files from Apache & NGINX if they exist
if [ -f "/etc/apache2/conf.d/conf" ]; then
	echo "[ * ] Removing old Apache configuration file from previous version of Control Panel..."
	rm -f /etc/apache2/conf.d/conf
fi
if [ -f "/etc/nginx/conf.d/conf" ]; then
	echo "[ * ] Removing old NGINX configuration file from previous version of Control Panel..."
