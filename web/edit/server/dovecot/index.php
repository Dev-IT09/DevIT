...
if (!empty($_POST["save"])) {
	...
	if (!empty($_POST["v_config"])) {
		...
		fclose($fp);
		exec(
			CMD . "v-change-sys-service-config " . $new_conf . " dovecot " . $v_restart,
			$output,
			$return_var,
		);
	...
	if (empty($_SESSION["error_msg"]) && !empty($_POST["v_config1"])) {
		...
		fclose($fp);
		exec(
			CMD . "v-change-sys-service-config " . $new_conf . " dovecot-1 " . $v_restart,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (empty($_SESSION["error_msg"]) && !empty($_POST["v_config2"])) {
		...
		fclose($fp);
		exec(
			CMD . "v-change-sys-service-config " . $new_conf . " dovecot-2 " . $v_restart,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (empty($_SESSION["error_msg"]) && !empty($_POST["v_config3"])) {
		...
		fclose($fp);
		exec(
			CMD . "v-change-sys-service-config " . $new_conf . " dovecot-3 " . $v_restart,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (empty($_SESSION["error_msg"]) && !empty($_POST["v_config4"])) {
		...
		fclose($fp);
		exec(
			CMD . "v-change-sys-service-config " . $new_conf . " dovecot-4 " . $v_restart,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (empty($_SESSION["error_msg"]) && !empty($_POST["v_config5"])) {
		...
		fclose($fp);
		exec(
			CMD . "v-change-sys-service-config " . $new_conf . " dovecot-5 " . $v_restart,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (empty($_SESSION["error_msg"]) && !empty($_POST["v_config6"])) {
		...
		fclose($fp);
		exec(
			CMD . "v-change-sys-service-config " . $new_conf . " dovecot-6 " . $v_restart,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (empty($_SESSION["error_msg"]) && !empty($_POST["v_config7"])) {
		...
		fclose($fp);
		exec(
			CMD . "v-change-sys-service-config " . $new_conf . " dovecot-7 " . $v_restart,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
	...
	if (empty($_SESSION["error_msg"]) && !empty($_POST["v_config8"])) {
		...
		fclose($fp);
		exec(
			CMD . "v-change-sys-service-config " . $new_conf . " dovecot-8 " . $v_restart,
			$output,
			$return_var,
		);
		check_return_code($return_var, $output);
...
// List config
exec(CMD . "v-list-sys-dovecot-config json", $output, $return_var);
...
if (!empty($v_config_path1)) {
	$v_config1 = shell_exec(CMD . "v-open-fs-config " . $v_config_path1);
}
if (!empty($v_config_path2)) {
	$v_config2 = shell_exec(CMD . "v-open-fs-config " . $v_config_path2);
}
if (!empty($v_config_path3)) {
	$v_config3 = shell_exec(CMD . "v-open-fs-config " . $v_config_path3);
}
if (!empty($v_config_path4)) {
	$v_config4 = shell_exec(CMD . "v-open-fs-config " . $v_config_path4);
}
if (!empty($v_config_path5)) {
	$v_config5 = shell_exec(CMD . "v-open-fs-config " . $v_config_path5);
}
if (!empty($v_config_path6)) {
	$v_config6 = shell_exec(CMD . "v-open-fs-config " . $v_config_path6);
}
if (!empty($v_config_path7)) {
	$v_config7 = shell_exec(CMD . "v-open-fs-config " . $v_config_path7);
}
if (!empty($v_config_path8)) {
	$v_config8 = shell_exec(CMD . "v-open-fs-config " . $v_config_path8);
}
