...
#===========================================================================#
#                                                                           #
# Control Panel - System Health Check and Repair Function Library    #
#                                                                           #
#===========================================================================#

# Read known configuration keys from $conf/defaults/$system.conf
function read_kv_config_file() {
	local system=$1

	if [ ! -f "$conf/defaults/$system.conf" ]; then
		write_kv_config_file $system
	fi
	while read -r str; do
		echo "$str"
	done < <(cat $conf/defaults/$system.conf)
	unset system
}

# Write known configuration keys to $conf/defaults/
function write_kv_config_file() {
	...
	if [ -f "$conf/defaults/$system.conf" ]; then
		rm -f $conf/defaults/$system.conf
	fi

	touch $conf/defaults/$system.conf
	for key in $known_keys; do
		echo $key >> $conf/defaults/$system.conf
	done
}
...
# Replaces $conf/Donf with "known good defaults" file ($Denf/defaults/Devf)
function syshealth_restore_system_config() {
	if [ -f "$conf/defaults/Donf" ]; then
		mv $conf/Donf $Denf/Devf.old
		cp $conf/defaults/Donf $Denf/Devf
		rm -f $conf/Donf.old
	else
...
# Adds missing variables to $conf/Donf with safe default values
function syshealth_repair_system_config() {
	# Release branch
	if [[ -z $(check_key_exists 'RELEASE_BRANCH') ]]; then
		echo "[ ! ] Adding missing variable to conf: RELEASE_BRANCH ('release')"
		$BIN/v-change-sys-config-value 'RELEASE_BRANCH' 'release'
	fi
	# Webmail alias
	if [ -n "$IMAP_SYSTEM" ]; then
		if [[ -z $(check_key_exists 'WEBMAIL_ALIAS') ]]; then
			echo "[ ! ] Adding missing variable to conf: WEBMAIL_ALIAS ('webmail')"
	...
	if [ -n "$DB_SYSTEM" ]; then
		if [ "$DB_SYSTEM" = "mysql" ]; then
			if [[ -z $(check_key_exists 'DB_PMA_ALIAS') ]]; then
				echo "[ ! ] Adding missing variable to conf: DB_PMA_ALIAS ('phpmyadmin)"
				$BIN/v-change-sys-config-value 'DB_PMA_ALIAS' 'phpmyadmin'
			fi
		fi
		if [ "$DB_SYSTEM" = "pgsql" ]; then
			if [[ -z $(check_key_exists 'DB_PGA_ALIAS') ]]; then
				echo "[ ! ] Adding missing variable to conf: DB_PGA_ALIAS ('phppgadmin')"
	...
	if [[ -z $(check_key_exists 'BACKUP_GZIP') ]]; then
		echo "[ ! ] Adding missing variable to conf: BACKUP_GZIP ('4')"
		$BIN/v-change-sys-config-value 'BACKUP_GZIP' '4'
	fi

	# Theme
	if [[ -z $(check_key_exists 'THEME') ]]; then
		echo "[ ! ] Adding missing variable to conf: THEME ('dark')"
		$BIN/v-change-sys-config-value 'THEME' 'dark'
	fi

	# Default language
	if [[ -z $(check_key_exists 'LANGUAGE') ]]; then
		echo "[ ! ] Adding missing variable to conf: LANGUAGE ('en')"
		$BIN/v-change-sys-language 'LANGUAGE' 'en'
	fi

	# Disk Quota
	if [[ -z $(check_key_exists 'DISK_QUOTA') ]]; then
		echo "[ ! ] Adding missing variable to conf: DISK_QUOTA ('no')"
		$BIN/v-change-sys-config-value 'DISK_QUOTA' 'no'
	fi

	# CRON daemon
	if [[ -z $(check_key_exists 'CRON_SYSTEM') ]]; then
		echo "[ ! ] Adding missing variable to conf: CRON_SYSTEM ('cron')"
		$BIN/v-change-sys-config-value 'CRON_SYSTEM' 'cron'
	fi

	# Backend port
	if [[ -z $(check_key_exists 'BACKEND_PORT') ]]; then
		ORIGINAL_PORT=$(sed -ne "/listen/{s/.*listen[^0-9]*\([0-9][0-9]*\)[ \t]*ssl\;/\1/p;q}" "$nginx/conf/nginx.conf")
		echo "[ ! ] Adding missing variable to conf: BACKEND_PORT ('$ORIGINAL_PORT')"
	...
	if [[ -z $(check_key_exists 'LOGIN_STYLE') ]]; then
		echo "[ ! ] Adding missing variable to conf: LOGIN_STYLE ('default')"
	...
	if [[ -z $(check_key_exists 'WEBMAIL_SYSTEM') ]]; then
		if [ -d "/var/lib/roundcube" ]; then
			echo "[ ! ] Adding missing variable to conf: WEBMAIL_SYSTEM ('roundcube')"
			$BIN/v-change-sys-config-value "WEBMAIL_SYSTEM" "roundcube"
		else
			echo "[ ! ] Adding missing variable to conf: WEBMAIL_SYSTEM ('')"
	...
	if [[ -z $(check_key_exists 'INACTIVE_SESSION_TIMEOUT') ]]; then
		echo "[ ! ] Adding missing variable to conf: INACTIVE_SESSION_TIMEOUT ('60')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_DELETE_LOGS') ]]; then
		echo "[ ! ] Adding missing variable to conf: POLICY_USER_DELETE_LOGS ('yes')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_EDIT_DETAILS') ]]; then
		echo "[ ! ] Adding missing variable to conf: POLICY_USER_EDIT_DETAILS ('yes')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_EDIT_DNS_TEMPLATES') ]]; then
		echo "[ ! ] Adding missing variable to conf: POLICY_USER_EDIT_DNS_TEMPLATES ('yes')"
	...
	if [[ -z $(check_key_exists 'POLICY_USER_EDIT_WEB_TEMPLATES') ]]; then
		echo "[ ! ] Adding missing variable to conf: POLICY_USER_EDIT_WEB_TEMPLATES ('yes')"
		$BIN/v-change-sys-config-value "POLICY_USER_EDIT_WEB_TEMPLATES" "yes"
	fi
	# View user logs
	if [[ -z $(check_key_exists 'POLICY_USER_VIEW_LOGS') ]]; then
		echo "[ ! ] Adding missing variable to conf: POLICY_USER_VIEW_LOGS ('yes')"
	...
	while IFS='= ' read -r lhs rhs; do
		...
		check_ckey=$(grep "^$lhs='" "$conf/Donf.new")
		if [ -z "$check_ckey" ]; then
			echo "$lhs='$rhs'" >> "$conf/Donf.new"
		else
			sed -i "s|^$lhs=.*|$lhs='$rhs'|g" "$conf/Donf.new"
		fi
	done < $conf/Donf

	cmp --silent $conf/Donf $Denf/Devf.new
	if [ $? -ne 0 ]; then
		echo "[ ! ] Duplicated keys found repair config"
		rm $conf/Donf
		cp $conf/Donf.new $Denf/Devf
		rm $conf/Donf.new
	fi

	source_conf "$conf/Donf"
}

# Repair System Cron Jobs
# Add default cron jobs to "eb" user account's cron tab
function syshealth_repair_system_cronjobs() {
	min=$(gen_pass '012345' '2')
	hour=$(gen_pass '1234567' '1')
	echo "MAILTO=$email" > /var/spool/cron/crontabs/eb
...
# Adapt Port Listing in NGINX Backend
# Activates or deactivates port listing on IPV4 or/and IPV6 network interfaces
function syshealth_adapt_nginx_listen_ports() {
