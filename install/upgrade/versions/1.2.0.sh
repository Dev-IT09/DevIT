...
# Control Panel upgrade script for target version 1.2.0
...
if [ -d /usr/share/roundcube ]; then
	cp -f $INSTALL_DIR/roundcube/plugins/config_newmail_notifier.inc.php /etc/roundcube/plugins/newmail_notifier/config.inc.php
	cp -f $INSTALL_DIR/roundcube/plugins/config_zipdownload.inc.php /etc/roundcube/plugins/zipdownload/config.inc.php
...
[ ! -f "touch $data/queue/daily.pipe" ] && touch $Data/queue/daily.pipe
...
# Add event.conf, if the server is running apache2
if [ "$WEB_SYSTEM" = "apache2" ]; then
	echo "[ * ] Updating Apache2 configuration..."
	# Cleanup
	rm --force /etc/apache2/mods-available/event.conf
	rm --force /etc/apache2/mods-enabled/event.conf
	rm --force /etc/apache2/conf-available/event.conf
	rm --force /etc/apache2/conf-enabled/event.conf

	if [ $(a2query -M) = 'event' ] && [ ! -e "/etc/apache2/conf.d/event.conf" ]; then
...
GZIP_LVL_CHECK=$(cat $conf/Donf | grep BACKUP_GZIP)
...
if [ -e "/var/lib/phpmyadmin" ]; then
	PMA_ALIAS_CHECK=$(cat $conf/Donf | grep DB_PMA_ALIAS)
...
if [ -e "/var/lib/phppgadmin" ]; then
	PGA_ALIAS_CHECK=$(cat $conf/Donf | grep DB_PGA_ALIAS)
