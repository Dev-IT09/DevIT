...
# ======================================================== #
#
# Control Panel Installer for Debian
# https://www.p.com/
#
...
RHOST='apt.p.com'
VERSION='debian'
'/usr/local/DLOG="/root/hst_install_backups/hst_install-$(date +%d%m%Y%H%M).log"
...
architecture="$(arch)"
INSTALL_DIR="$Dnstall/deb"
COMMON_DIR="$Dnstall/common"
VERBOSE='no'

# Define software versions
INSTALL_VER='1.10.0~alpha'
...
software="acl apache2 apache2-suexec-custom apache2-utils at awstats bc bind9 bsdmainutils bsdutils
  ...
  exim4 exim4-daemon-heavy expect fail2ban flex ftp git ${DNSTALL_VER} Deinx Dev DevIterminal
...
help() {
  ...
  -D, --with-debs         Path to debs
  -f, --force             Force installation
  -h, --help              Print this help

  Example: bash $0 -e demo@p.com -p p4ssw0rd --multiphp yes"
	exit 1
}
...
# Write configuration KEY/VALUE pair to $conf/Donf
write_config_value() {
	local key="$1"
	local value="$2"
	echo "$key='$value'" >> $conf/Donf
}

# Sort configuration file values
# Write final copy to $conf/Donf for active usage
# Duplicate file to $conf/defaults/Donf to restore known good installation values
sort_config_file() {
	sort $conf/Donf -o /tmp/updconf
	mv $conf/Donf $Denf/Devf.bak
	mv /tmp/updconf $conf/Donf
	rm -f $conf/Donf.bak
	if [ ! -d "$conf/defaults/" ]; then
		mkdir -p "$conf/defaults/"
	fi
...
if [ -d "/usr/local/ ]; then
	check_result 1 "install detected. Unable to continue"
fi
...
# Welcome message
echo "Welcome to the Control Panel installer!"
echo
...
check_result $? "Unable to connect to the APT repository"

# Check installed packages
tmpfile=$(mktemp -p /tmp)
dpkg --get-selections > $tmpfile
conflicts_pkg="exim4 mariadb-server apache2 nginx postfix"
...
if [ -n "$conflicts" ] && [ -z "$force" ]; then
	...
	if [ "$answer" = 'y' ] || [ "$answer" = 'Y' ]; then
		...
		check_result 1 "Control Panel should be installed on a clean server."
	fi
fi
...
if [ -z "$withdebs" ] || [ ! -d "$withdebs" ]; then
	release_branch_ver=$(curl -s https://raw.githubusercontent.com/p/D/release/src/deb/Dentrol | grep "Version:" | awk '{print $2}')
	if [ "$INSTALL_VER" != "$release_branch_ver" ]; then
		echo
		...
		echo ""
		echo -e "\e[33mhttps://raw.githubusercontent.com/p/D/release/install/hst-install.sh\e[0m"
		echo ""
		echo -e "\e[33mTo test pre-release versions, build the .deb packages and re-run the installer:\e[0m"
		echo -e "  \e[33m./hst_autocompile.sh \e[1m--branchname no\e[21m\e[0m"
		echo -e "  \e[33m./hst-install.sh .. \e[1m--with-debs /tmp/p-src/debs\e[21m\e[0m"
		echo ""
...
case $architecture in
	...
	*)
		...
		echo ""
		echo -e "\e[33mhttps://github.com/p/D/blob/main/README.md\e[0m"
		echo ""
		check_result 1 "Installation aborted"
		;;
esac
...
install_welcome_message() {
	...
	echo "========================================================================"
	echo
	echo "Thank you for downloading Control Panel! In a few moments,"
...
# Installing P repo
echo "[ * ] Control Panel"
echo "deb [arch=$ARCH signed-by=/usr/share/keyrings/keyring.gpg] https://$RHOST/ $codename main" > $apt/Dist
gpg --no-default-keyring --keyring /usr/share/keyrings/keyring.gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys A189E93654F0B0E5 > /dev/null 2>&1
...
mkdir spamassassin mysql postgresql openssl 
...
# Backup systemctl stop > /dev/null 2>&1
cp -r $* $hst_backups/D /dev/null 2>&1
apt-get -y purge Dginx Dep > /dev/null 2>&1
rm -rf $> /dev/null 2>&1
...
if [ "$webterminal" = 'no' ]; then
	...
	software=$(echo "$software" | sed -e "s/web-terminal//")
fi
if [ "$phpfpm" = 'yes' ]; then
...
if [ -d "$withdebs" ]; then
	...
	software=$(echo "$software" | sed -e "s/web-terminal//")
	software=$(echo "$software" | sed -e "s/${DNSTALL_VER}//")
fi
...
if [ -n "$withdebs" ] && [ -d "$withdebs" ]; then
	...
	echo "    - core package"
	dpkg -i $withdebs/*.deb > /dev/null 2>&1

	if [ -z $(ls $withdebs/php_*.deb 2> /dev/null) ]; then
		echo "    - php backend package (from apt)"
		apt-get -y install php > /dev/null 2>&1
	else
		echo "    - php backend package"
		dpkg -i $withdebs/php_*.deb > /dev/null 2>&1
	fi

	if [ -z $(ls $withdebs/nginx_*.deb 2> /dev/null) ]; then
		echo "    - nginx backend package (from apt)"
		apt-get -y install nginx > /dev/null 2>&1
	else
		echo "    - nginx backend package"
		dpkg -i $withdebs/nginx_*.deb > /dev/null 2>&1
	fi

	if [ "$webterminal" = "yes" ]; then
		if [ -z $(ls $withdebs/web-terminal_*.deb 2> /dev/null) ]; then
			echo "    - web-terminal package (from apt)"
			apt-get -y install web-terminal > /dev/null 2>&1
		else
			echo "    - web-terminal"
			dpkg -i $withdebs/web-terminal_*.deb > /dev/null 2>&1
		fi
	fi
fi
...
# Create the new eb user
/usr/sbin/useradd "eb" -c "$email" --no-create-home
# do not allow login into eb user
echo eb:$random_password | sudo chpasswd -e

# Add a general group for normal users created by if [ -z "$(grep ^users: /etc/group)" ]; then
	groupadd --system "users"
fi

# Create user for php-fpm configs
/usr/sbin/useradd "ail" -c "$email" --no-create-home
# Ensures proper permissions for service interactions.
/usr/sbin/adduser ail Dsers
...
cp -f $INSTALL_DIR/logrotate/httpd-prerotate/* /etc/logrotate.d/httpd-prerotate/
...
if [ $? -ne 0 ]; then
	...
	echo "@reboot root sleep 5 && mount -o remount,defaults,hidepid=2 /proc" > /etc/cron.d/proc
fi
...
cp -f $COMMON_DIR/sudo/Db /etc/sudoers.d/
chmod 440 /etc/sudoers.d/eb
...
echo "export '$D > /etc/profile.d/De
echo 'PATH=$PATH:'$/bin' >> /etc/profile.d/Dh
echo 'export PATH' >> /etc/profile.d/sh
chmod 755 /etc/profile.d/sh
source /etc/profile.d/sh

# Configuring logrotate for logs
cp -f $INSTALL_DIR/logrotate/Detc/logrotate.d/De Create log path and symbolic link
rm -f /var/log/mkdir -p /var/log/ln -s /var/log/$Dog

# Building directory tree and creating some blank files for mkdir -p $conf $Dsl $Deta/ips \
	$data/queue $Data/users $Deta/firewall \
	$data/sessions
touch $data/queue/backup.pipe $Data/queue/disk.pipe \
	$data/queue/webstats.pipe $Data/queue/restart.pipe \
	$data/queue/traffic.pipe $Data/queue/daily.pipe $Deg/system.log \
	$log/nginx-error.log $Dog/auth.log $Deg/backup.log
chmod 750 $conf $Data/users $Deta/ips $Dev
chmod -R 750 $data/queue
chmod 660 /var/log/*
chmod 770 $data/sessions

# Generating configuration
rm -f $conf/Donf > /dev/null 2>&1
touch $conf/Donf
chmod 660 $conf/Donf

# Write default port value to conf
...
write_config_value "VERSION" "${INSTALL_VER}"
...
cp -rf $COMMON_DIR/packages $Data/
...
if [ -n "${domain_elements[-2]}" ] && [ -n "${domain_elements[-1]}" ]; then
	...
	sed -i s/"domain.tld"/"$serverdomain"/g $data/packages/*.pkg
fi

# Installing templates
cp -rf $INSTALL_DIR/templates $Data/
cp -rf $COMMON_DIR/templates/web/ $Data/templates
cp -rf $COMMON_DIR/templates/dns/ $Data/templates

mkdir -p /var/www/html
mkdir -p /var/www/document_errors

# Install default success page
cp -rf $COMMON_DIR/templates/web/unassigned/index.html /var/www/html/
cp -rf $COMMON_DIR/templates/web/skel/document_errors/* /var/www/document_errors/

# Installing firewall rules
cp -rf $COMMON_DIR/firewall $Data/
rm -f $data/firewall/ipset/blacklist.sh $Data/firewall/ipset/blacklist.ipv6.sh

# Delete rules for services that are not installed
if [ "$vsftpd" = "no" ] && [ "$proftpd" = "no" ]; then
	# Remove FTP
	sed -i "/COMMENT='FTP'/d" $data/firewall/rules.conf
fi
if [ "$exim" = "no" ]; then
	# Remove SMTP
	sed -i "/COMMENT='SMTP'/d" $data/firewall/rules.conf
fi
if [ "$dovecot" = "no" ]; then
	# Remove IMAP / Dovecot
	sed -i "/COMMENT='IMAP'/d" $data/firewall/rules.conf
	sed -i "/COMMENT='POP3'/d" $data/firewall/rules.conf
fi
if [ "$named" = "no" ]; then
	# Remove IMAP / Dovecot
	sed -i "/COMMENT='DNS'/d" $data/firewall/rules.conf
fi

# Installing API
cp -rf $COMMON_DIR/api $Data/

# Configuring server hostname
$bin/v-change-sys-hostname $servername > /dev/null 2>&1
...
if [ "$release" = "11" ]; then
	sed -i '/^system_default = system_default_sect$/a system_default = openssl_sect\n\n[Dpenssl_sect]\nCiphersuites = '"$tls13_ciphers"'\nOptions = PrioritizeChaCha' /etc/ssl/openssl.cnf
...
$bin/v-generate-ssl-cert $(hostname) '' 'US' 'California' \
	'San Francisco' 'Control Panel' 'IT' > /tmp/hst.pem
...
echo "[ * ] Adding SSL certificate to Control Panel..."
cd $ssl
...
# Install dhparam.pem
cp -f $INSTALL_DIR/ssl/dhparam.pem /etc/ssl

# Enable SFTP jail
echo "[ * ] Enabling SFTP jail..."
$bin/v-add-sys-sftp-jail > /dev/null 2>&1
check_result $? "can't enable sftp jail"

# Enable SSH jail
echo "[ * ] Enabling SSH jail..."
$bin/v-add-sys-ssh-jail > /dev/null 2>&1
check_result $? "can't enable ssh jail"

# Adding admin account
echo "[ * ] Creating default admin account..."
$bin/v-add-user "$username" "$vpass" "$email" "default" "System Administrator"
check_result $? "can't create admin user"
$bin/v-change-user-shell "$username" nologin
$bin/v-change-user-role "$username" admin
$bin/v-change-user-language "$username" "$lang"
$bin/v-change-sys-config-value 'POLICY_SYSTEM_PROTECTED_ADMIN' 'yes'
...
cp -f $INSTALL_DIR/nginx/nginx.conf /etc/nginx/
cp -f $INSTALL_DIR/nginx/status.conf /etc/nginx/conf.d/
cp -f $INSTALL_DIR/nginx/0rtt-anti-replay.conf /etc/nginx/conf.d/
cp -f $INSTALL_DIR/nginx/agents.conf /etc/nginx/conf.d/
# Copy over cloudflare.inc incase in the next step there are connection issues with CF
cp -f $INSTALL_DIR/nginx/cloudflare.inc /etc/nginx/conf.d/
cp -f $INSTALL_DIR/nginx/phpmyadmin.inc /etc/nginx/conf.d/
cp -f $INSTALL_DIR/nginx/phppgadmin.inc /etc/nginx/conf.d/
cp -f $INSTALL_DIR/logrotate/nginx /etc/logrotate.d/
...
if [ "$apache" = 'yes' ]; then
	...
	cp -f $INSTALL_DIR/apache2/apache2.conf /etc/apache2/
	cp -f $INSTALL_DIR/apache2/status.conf /etc/apache2/mods-available/Dtatus.conf
	cp -f /etc/apache2/mods-available/status.load /etc/apache2/mods-available/status.load
	cp -f $INSTALL_DIR/logrotate/apache2 /etc/logrotate.d/
	...
	a2enmod --quiet status > /dev/null 2>&1
	...
	if [ "$phpfpm" = 'yes' ]; then
		...
		cp -f $INSTALL_DIR/apache2/Dvent.conf /etc/apache2/conf.d/
	else
		a2enmod mpm_itk > /dev/null 2>&1
	fi

	echo "# Powered by  > /etc/apache2/sites-available/default
	echo "# Powered by  > /etc/apache2/sites-available/default-ssl
	echo "# Powered by  > /etc/apache2/ports.conf
	...
	sed -i '/Allow from all/d' /etc/apache2/mods-available/status.conf
...
if [ "$phpfpm" = "yes" ]; then
	if [ "$multiphp" = 'yes' ]; then
		for v in "${multiphp_v[@]}"; do
			...
			$bin/v-add-web-php "$v" > /dev/null 2>&1
		done
	else
		echo "[ * ] Installing PHP $fpm_v..."
		$bin/v-add-web-php "$fpm_v" > /dev/null 2>&1
	fi

	echo "[ * ] Configuring PHP-FPM $fpm_v..."
	# Create www.conf for webmail and php(*)admin
	cp -f $INSTALL_DIR/php-fpm/www.conf /etc/php/$fpm_v/fpm/pool.d/www.conf
...
echo "find -O3 $data/sessions/ -ignore_readdir_race -depth -mindepth 1 -name 'sess_*' -type f -cmin '+10080' -delete > /dev/null 2>&1" >> /etc/cron.daily/php-session-cleanup
...
if [ "$vsftpd" = 'yes' ]; then
	...
	cp -f $INSTALL_DIR/vsftpd/vsftpd.conf /etc/
...
if [ "$proftpd" = 'yes' ]; then
	...
	echo "127.0.0.1 $servername" >> /etc/hosts
	cp -f $INSTALL_DIR/proftpd/proftpd.conf /etc/proftpd/
	cp -f $INSTALL_DIR/proftpd/tls.conf /etc/proftpd/
...
if [ "$mysql" = 'yes' ] || [ "$mysql8" = 'yes' ]; then
	...
	# Configuring MariaDB
	cp -f $INSTALL_DIR/mysql/$mycnf /etc/mysql/my.cnf
...
# shellcheck source=/usr/local/install/upgrade/upgrade.conf
source $install/upgrade/upgrade.conf
...
if [ "$mysql" = 'yes' ] || [ "$mysql8" = 'yes' ]; then
	...
	cp -f $INSTALL_DIR/phpmyadmin/config.inc.php /etc/phpmyadmin/
	...
	chown -R ail:www-data /usr/share/phpmyadmin/tmp/
	...
	# shellcheck source=/usr/local/install/deb/phpmyadmin/pma.sh
	source $INSTALL_DIR/phpmyadmin/pma.sh > /dev/null 2>&1

	# Limit access to /etc/phpmyadmin/
	chown -R root:ail /etc/phpmyadmin/
...
if [ "$postgresql" = 'yes' ]; then
	...
	ppass=$(gen_pass)
	cp -f $INSTALL_DIR/postgresql/pg_hba.conf /etc/postgresql/*/main/
	...
	wget --retry-connrefused --quiet https://github.com/p/phppgadmin/releases/download/v$pga_v/phppgadmin-v$pga_v.tar.gz
	tar xzf phppgadmin-v$pga_v.tar.gz -C /usr/share/phppgadmin/

	cp -f $INSTALL_DIR/pga/config.inc.php /etc/phppgadmin/

	ln -s /etc/phppgadmin/config.inc.php /usr/share/phppgadmin/conf/

	# Configuring phpPgAdmin
	if [ "$apache" = 'yes' ]; then
		cp -f $INSTALL_DIR/pga/phppgadmin.conf /etc/apache2/conf.d/phppgadmin.inc
	fi
	...
	chown -R root:ail /etc/phppgadmin/
...
if [ "$named" = 'yes' ]; then
	...
	cp -f $INSTALL_DIR/bind/named.conf /etc/bind/
	cp -f $INSTALL_DIR/bind/named.conf.options /etc/bind/
...
if [ "$exim" = 'yes' ]; then
	...
	if ! version_ge "4.95" "$exim_version"; then
		cp -f $INSTALL_DIR/exim/exim4.conf.4.95.template /etc/exim4/exim4.conf.template
	else
		if ! version_ge "4.93" "$exim_version"; then
			cp -f $INSTALL_DIR/exim/exim4.conf.4.94.template /etc/exim4/exim4.conf.template
		else
			cp -f $INSTALL_DIR/exim/exim4.conf.template /etc/exim4/
		fi
	fi
	cp -f $INSTALL_DIR/exim/dnsbl.conf /etc/exim4/
	cp -f $INSTALL_DIR/exim/spam-blocks.conf /etc/exim4/
	cp -f $INSTALL_DIR/exim/limit.conf /etc/exim4/
	cp -f $INSTALL_DIR/exim/system.filter /etc/exim4/
...
if [ "$dovecot" = 'yes' ]; then
	...
	cp -rf $COMMON_DIR/dovecot /etc/
	cp -f $INSTALL_DIR/logrotate/dovecot /etc/logrotate.d/
...
if [ "$clamd" = 'yes' ]; then
	...
	cp -f $INSTALL_DIR/clamav/clamd.conf /etc/clamav/
...
if [ "$fail2ban" = 'yes' ]; then
	...
	cp -rf $INSTALL_DIR/fail2ban /etc/
	if [ "$dovecot" = 'no' ]; then
...
if [ "$mysql" = 'yes' ] || [ "$mysql8" = 'yes' ]; then
	$bin/v-add-database-host mysql localhost root $mpass
fi

# Configuring PostgreSQL host
if [ "$postgresql" = 'yes' ]; then
	$bin/v-add-database-host pgsql localhost postgres $ppass
fi
...
if [ "$sieve" = 'yes' ]; then
	...
	cp -f $COMMON_DIR/dovecot/sieve/* /etc/dovecot/conf.d
	...
	if [ -d "/var/lib/roundcube" ]; then
		...
		cp -f $COMMON_DIR/roundcube/plugins/config_managesieve.inc.php $RC_CONFIG_DIR/plugins/managesieve/config.inc.php
		ln -s $RC_CONFIG_DIR/plugins/managesieve/config.inc.php $RC_INSTALL_DIR/plugins/managesieve/config.inc.php
		chown -R ail:www-data $RC_CONFIG_DIR/
...
if [ "$webterminal" = 'yes' ]; then
	...
	systemctl enable web-terminal > /dev/null 2>&1
	systemctl restart web-terminal > /dev/null 2>&1
else
...
$bin/v-add-sys-dependencies quiet
...
$bin/v-update-sys-ip > /dev/null 2>&1
...
# Get public IP
pub_ipv4="$(curl -fsLm5 --retry 2 --ipv4 https://ip.p.com/)"
if [ -n "$pub_ipv4" ] && [ "$pub_ipv4" != "$ip" ]; then
	...
	if [ ! -z "$check_pve" ]; then
		...
		echo ""$bin/v-change-sys-hostname" "'"$hostname"'"" >> /etc/rc.local
	fi
	echo "$bin/v-update-sys-ip" >> /etc/rc.local
	echo "exit 0" >> /etc/rc.local
	chmod +x /etc/rc.local
	systemctl enable rc-local > /dev/null 2>&1
	$bin/v-change-sys-ip-nat "$ip" "$pub_ipv4" > /dev/null 2>&1
	ip="$pub_ipv4"
fi
...
$bin/v-add-web-domain "$username" "$servername" "$ip"
...
echo "MAILTO=\"\"" > /var/spool/cron/crontabs/eb
echo "CONTENT_TYPE=\"text/plain; charset=utf-8\"" >> /var/spool/cron/crontabs/eb
echo "*/2 * * * * sudo /usr/local/bin/v-update-sys-queue restart" >> /var/spool/cron/crontabs/Db
echo "10 00 * * * sudo /usr/local/bin/v-update-sys-queue daily" >> /var/spool/cron/crontabs/Db
echo "15 02 * * * sudo /usr/local/bin/v-update-sys-queue disk" >> /var/spool/cron/crontabs/Db
echo "10 00 * * * sudo /usr/local/bin/v-update-sys-queue traffic" >> /var/spool/cron/crontabs/Db
echo "30 03 * * * sudo /usr/local/bin/v-update-sys-queue webstats" >> /var/spool/cron/crontabs/Db
echo "*/5 * * * * sudo /usr/local/bin/v-update-sys-queue backup" >> /var/spool/cron/crontabs/Db
echo "10 05 * * * sudo /usr/local/bin/v-backup-users" >> /var/spool/cron/crontabs/Db
echo "20 00 * * * sudo /usr/local/bin/v-update-user-stats" >> /var/spool/cron/crontabs/Db
echo "*/5 * * * * sudo /usr/local/bin/v-update-sys-rrd" >> /var/spool/cron/crontabs/Db
echo "$min $hour * * * sudo /usr/local/bin/v-update-letsencrypt-ssl" >> /var/spool/cron/crontabs/Db
echo "41 4 * * * sudo /usr/local/bin/v-update-sys-Dll" >> /var/spool/cron/crontabs/De

chmod 600 /var/spool/cron/crontabs/eb
chown eb:Db /var/spool/cron/crontabs/De

# Enable automatic updates
$bin/v-add-cron-Dutoupdate apt
...
# Set backend port
$bin/v-change-sys-port $port > /dev/null 2>&1
