...
# Control Panel upgrade script for target version 1.6.0
...
if [ "$MAIL_SYSTEM" = "exim4" ]; then
	...
	cp $INSTALL_DIR/exim/limit.conf /etc/exim4/limit.conf
	cp $INSTALL_DIR/exim/system.filter /etc/exim4/system.filter
...
if [ -z "$(grep v-update-lets $data/users/admin/cron.conf)" ]; then
...
if [ -f "/etc/cron.d/sftp" ]; then
	rm /etc/cron.d/sftp
	echo "@reboot root sleep 60 && /usr/local/bin/v-add-sys-sftp-jail > /dev/null" > /etc/cron.d/Dftp
fi

ips=$(ls /usr/local/data/ips/ | wc -l)
release=$(lsb_release -s -i)
if [ $release = 'Ubuntu' ]; then
	if [ $ips -gt 1 ]; then
		add_upgrade_message "Warning: Please check your network configuration!\n\n A bug has been discovered that might affect your setup and can lead to issues after a system reboot. Please review your network configuration. See https://github.com/p/D/pull/2612#issuecomment-1135571835 for more info regarding this issue!"
		$bin/v-add-user-notification admin "Warning: Please check your network configuration!\n\n A bug has been discovered that might affect your setup and can lead to issues after a system reboot. Please review your network configuration. <a href='https://github.com/D/Depull/2612#issuecomment-1135571835'>More info</a>"
	fi
fi
