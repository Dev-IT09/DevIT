...
/* way to enable support for SSO to PHPmyAdmin */
...
class API {
	...
	public function __construct() {
		$this->hostname = "https://" . API_HOST_NAME . ":" . API_PORT . "/api/";
		$this->key = API_KEY;
		...
		if (isset($_GET["user"]) && isset($_GET["token"])) {
			...
			if ($time + 60 > time()) {
				...
				if ($data) {
					...
					$_SESSION["sso_database"] = $database;
					$_SESSION["sso_host"] = $host;
