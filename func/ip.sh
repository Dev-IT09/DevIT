...
#===========================================================================#
#                                                                           #
# Control Panel - IP/Network Function Library                        #
#                                                                           #
#===========================================================================#
...
# Check ip ownership
is_ip_owner() {
	owner=$(grep 'OWNER=' $data/ips/$ip | cut -f 2 -d \')
	if [ "$owner" != "$user" ]; then
...
# Get ip name
get_ip_alias() {
	ip_name=$(grep "NAME=" $data/ips/$local_ip | cut -f 2 -d \')
	if [ -n "$ip_name" ]; then
		echo "${1//./-}.$ip_name"
	fi
}
...
increase_ip_value() {
	...
	usr_key='U_SYS_USERS'
	current_web=$(grep "$web_key=" $data/ips/$sip | cut -f 2 -d \')
	current_usr=$(grep "$usr_key=" $data/ips/$sip | cut -f 2 -d \')
...
# Get real ip address
get_real_ip() {
	if [ -e "$data/ips/$1" ]; then
		echo "$1"
	else
		nat=$(grep -H "^NAT='$1'" $data/ips/* | head -n1)
		if [ -n "$nat" ]; then
...
# Get user ips
get_user_ips() {
	dedicated=$(grep -H "OWNER='$user'" $data/ips/*)
	dedicated=$(echo "$dedicated" | cut -f 1 -d : | sed 's=.*/==' | grep -E ${REGEX_IPV4})
	shared=$(grep -H -A1 "OWNER='$ROOT_USER'" $data/ips/* | grep shared)
...
get_user_ip() {
	...
	local_ip=$ip
	nat=$(grep "^NAT" $data/ips/$ip | cut -f 2 -d \')
	if [ -n "$nat" ]; then
		ip=$nat
	fi
}
...
is_ip_valid() {
	...
	if [ -n "$2" ]; then
		if [ -z "$nat" ]; then
			ip_data=$(cat $data/ips/$1)
		else
			ip_data=$(cat $data/ips/$nat)
		fi
