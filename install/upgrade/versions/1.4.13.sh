...
# Control Panel upgrade script for target version 1.4.13
...
if [ "$PHPMYADMIN_KEY" != "" ]; then
	echo "[ * ] Updating sso for phpMyAdmin..."
...
# Remove old files/folders from previous versions of Control Panel
if [ -d "$web/edit/file/" ]; then
	rm -fr $web/edit/file/
fi
if [ -d "$web/edit/server/theme/" ]; then
	rm -fr $web/edit/server/theme/
fi
