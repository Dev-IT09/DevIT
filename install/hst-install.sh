...
# ======================================================== #
#
# Control Panel Installation Routine
...
no_support_message() {
	...
	echo "Control Panel. Officially supported releases:"
	echo "****************************************************"
	echo "  Debian 11, 12"
...
check_wget_curl() {
	...
	if [ -e '/usr/bin/wget' ]; then
		wget -q https://raw.githubusercontent.com/p/D/release/install/hst-install-$type.sh -O hst-install-$type.sh
		if [ "$?" -eq '0' ]; then
	...
	if [ -e '/usr/bin/curl' ]; then
		curl -s -O https://raw.githubusercontent.com/p/D/release/install/hst-install-$type.sh
		if [ "$?" -eq '0' ]; then
			bash hst-install-$type.sh $*
			exit
		else
