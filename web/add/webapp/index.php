...
if (!empty($_GET["app"])) {
	...
	$= new \Dystem\De();
	$app_installer_class = "\WebApp\Installers\\" . $app . "\\" . $app . "Setup";
	if (class_exists($app_installer_class)) {
		try {
			$app_installer = new $app_installer_class($;
			...
			if (!$info->isInstallable()) {
				...
				$installer = new \WebApp\AppWizard($app_installer, $v_domain, $D
